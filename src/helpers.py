...
def plotScene(X, scenario, vehicle, vehList):
    fig = plt.figure('Episode Simulation')
    ax = fig.add_subplot(111)

    # Get road and vehicle specifications
    Nveh = len(vehList)
    roadMin, roadMax, laneCenters = scenario.getRoad()
    vehWidth, length, L_tract, L_trail = vehicle.getSize()
    leadWidth, leadLength = vehList[0].getSize()

    plt.cla()
    # Plot Road
    frameSize = 70
    X_road = np.array([X[0, 0, 0] - frameSize, X[0, 0, 0] + frameSize])

    plt.plot(X_road, np.ones(2, ) * roadMax / 2, '--', color='r')
    plt.plot(X_road, np.zeros(2, ), '--', color='r')
    plt.plot(X_road, np.ones(2, ) * roadMin, '-', color='k')
    plt.plot(X_road, np.ones(2, ) * roadMax, '-', color='k')

    # Plot ego vehicle
    X_new = rotmatrix([0, -vehWidth / 2], [X[0, 0, 0], X[1, 0, 0]], X[3, 0, 0])
    tractor = Rectangle((X_new[0], X_new[1]), width=L_tract, height=vehWidth, angle=180 * X[3, 0, 0] / np.pi,
                        linewidth=1, edgecolor='k', facecolor='c', fill=True)

    ax.add_patch(tractor)

    plt.scatter(X[0, 0, 0], X[1, 0, 0])

    X_new = rotmatrix([-L_trail, -vehWidth / 2], [X[0, 0, 0], X[1, 0, 0]], X[4, 0, 0])
    trailer = Rectangle((X_new[0], X_new[1]), width=L_trail, height=vehWidth, angle=180 * (X[4, 0, 0]) / np.pi,
                        linewidth=1, edgecolor='k', facecolor='c', fill=True)
    ...
    for j in range(1, Nveh + 1):
        color = colors[vehList[j - 1].type]
        ax.add_patch(Rectangle(
            xy=(X[0, 0, j] - leadLength / 2, X[1, 0, j] - leadWidth / 2), width=leadLength, height=leadWidth,
            angle=180 * X[3, 0, j] / np.pi, linewidth=1, edgecolor='k',
            facecolor=color, fill=True))

    plt.axis('equal')
    plt.xlim(X[0, 0, 0] - frameSize, X[0, 0, 0] + frameSize)
        ...
        def animate(i):
            ...
            for j in range(Nveh):
                ...
                axanime.add_patch(Rectangle(
                    xy=(X_traffic[0, i, j] - leadLength / 2, X_traffic[1, i, j] - leadWidth / 2), width=leadLength,
                    height=leadWidth,
